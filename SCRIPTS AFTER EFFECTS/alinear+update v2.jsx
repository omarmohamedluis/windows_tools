...
(function(){
    ...
    function buildUI(comp){
        ...
        // Checkbox: Forzar lineal (temporal y espacial)
        var cbLinear = win.add("checkbox", undefined, "Forzar lineal (temporal y espacial)");
        cbLinear.value = true; // por defecto activado
        ...
        return {
            ...
            getForceLinear: function(){ return cbLinear.value; }
        };
    }
    ...
    // ---- helpers de lineal ----
    function zeroArrayLike(arr){
        if (!arr) return null;
        var out = [];
        for (var i=0; i<arr.length; i++) out.push(0);
        return out;
    }
    function forceLinearAtKey(prop, idx){
        // Temporal lineal
        try {
        ...
        try { prop.setTemporalAutoBezierAtKey(idx, false); } catch(e){}
        try { prop.setTemporalContinuousAtKey(idx, false); } catch(e){}
        try {
            var z = [new KeyframeEase(0,0)];
            prop.setTemporalEaseAtKey(idx, z, z);
        } catch(e){}

        // Espacial lineal (si aplica: posición, etc.)
        try {
            var inT = prop.keyInSpatialTangent(idx);
            var outT = prop.keyOutSpatialTangent(idx);
            if (inT && outT){
                prop.setSpatialTangentsAtKey(idx, zeroArrayLike(inT), zeroArrayLike(outT));
            }
        } catch(e){}
        try { prop.setSpatialAutoBezierAtKey(idx, false); } catch(e){}
        try { prop.setSpatialContinuousAtKey(idx, false); } catch(e){}
    }

    function remapKeysToRange(prop, tStart, tEnd, forceLinear){
        ...
        // Recrear keys con todos sus atributos
        ...
        for (var n=0; n<kNum; n++){
            ...
            // OVERRIDE a lineal si está marcado
            if (forceLinear) forceLinearAtKey(prop, idx);
        }

        // Doble garantía: si se fuerza lineal, repasar todas las keys
        if (forceLinear){
            for (var q=1; q<=prop.numKeys; q++) forceLinearAtKey(prop, q);
        }
    ...
    var forceLinear = ui.getForceLinear();
    ...
    for (var i=0; i<maxPairsUsables; i++){
        ...
        if ((!prop.isTimeVarying || prop.numKeys < 2) && prop.numProperties){
            ...
            for (var sp=1; sp<=prop.numProperties; sp++){
                ...
                if (remapKeysToRange(sub, range.IN, range.OUT, forceLinear)){ ok = true; adjusted++; }
            }
            if (!ok) skipped++;
        } else {
            if (remapKeysToRange(prop, range.IN, range.OUT, forceLinear)) adjusted++;
            else skipped++;
        }
    }

    alert("Hecho.\nReferencia: " + refLayer.name +
          "\nForzar lineal: " + (forceLinear ? "Sí" : "No") +
