// Marcadores→Frases_porCapa_precompUpdate_reset03_refresh.jsx
...
// - "Resetear tiempos (0s/3s)" en las capas seleccionadas
// - NUEVO: "Actualizar capas" para recargar el desplegable tras cambiar de comp
// Compatible ExtendScript (AE 2024/2025). Sin let/const ni palabras reservadas.
...
(function (thisObj) {
    ...
    function resetTimes_0_3_forSelectedLayers(log){
        ...
        function normalizeProp(prop){
            ...
            for (j=0; j<keys.length; j++){
                var k = keys[j];
                var rel = k.t - srcStart;
                var newT = dstStart + rel * scale; // t1->0, t2->3, los demás proporcionados
                var idx = prop.addKey(newT);
                prop.setValueAtKey(idx, k.v);
                if (k.inI && k.outI) prop.setInterpolationTypeAtKey(idx, k.inI, k.outI);
                if (k.inE && k.outE) prop.setTemporalEaseAtKey(idx, k.inE, k.outE);
    ...
    function buildUI(thisObj){
        ...
        // Fila: dropdown + NUEVO botón actualizar capas
        var rowRef = g.add("group"); rowRef.orientation = "row"; rowRef.alignChildren = "left";
        rowRef.add("statictext", undefined, "Capa de referencia:");
        var ddRef = rowRef.add("dropdownlist", undefined, []);
        ddRef.minimumSize.width = 300;
        var btnReloadLayers = rowRef.add("button", undefined, "Actualizar capas"); // NUEVO
        ...
        btnReloadLayers.onClick = function(){ populateRefDropdown(); }; // NUEVO
